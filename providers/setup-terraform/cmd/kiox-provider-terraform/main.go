...
func main() {
	...
	fs.StringVar(&config.ToolName, "tool-name", firstNonEmpty(os.Getenv("KIOX_TARGET_TOOL_NAME"), os.Getenv("TINX_TARGET_TOOL_NAME"), "terraform"), "tool name to materialize")
	...
	if len(downloadMirrors) > 0 {
		...
		config.DownloadMirrors = provider.URLsFromEnv(os.Getenv("TERRAFORM_DOWNLOAD_BASE_URLS"))
	}
	if len(indexURLs) > 0 {
		config.IndexURLs = append(config.IndexURLs[:0], indexURLs...)
	} else {
		config.IndexURLs = provider.URLsFromEnv(os.Getenv("TERRAFORM_INDEX_URLS"))
	}
